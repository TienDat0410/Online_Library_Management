...
const productsCtrl = {
   ...
   /*postbook: (req, res) => {
        let data = req.body;
        let sql = "INSERT INTO sach (MaSach,TenSach,TacGia,MaTheLoai,MaNXB,DonGia,SoLuongTon,SoLanMuon,TinhTrang) values ('${MaSach}','${TenSach}','${TacGia}','${MaTheLoai}','${MaNXB}','${DonGia}','${SoLuongTon}','${SoLanMuon}','${TinhTrang}')"
        db.query(sql, [data], (err, response) => {
            if (err) throw err
            res.send({ message: 'Insert success!' })
        })
